...
mod tests {
    ...
    #[test]
    fn fixed_silence_response_keeps_input_id() {
        let request = Request {
            v: 1,
            id: "r1".to_owned(),
        ...
        assert_eq!(response["result"]["action"], "silence");
    }

    #[test]
    fn fixed_plugin_supports_repeated_feed_and_commit_operations() {
        let feed = response_for(
            Request {
                v: 1,
                id: "r2".to_owned(),
                session: Some("s1".to_owned()),
        ...
        assert_eq!(feed["result"]["applied"], true);
        assert_eq!(commit["result"]["committed"], true);
    }
    ...
    fn resident_loop_returns_one_response_for_each_request() {
        ...
        run(Cursor::new(input), &mut output, &Options::default()).expect("run");
