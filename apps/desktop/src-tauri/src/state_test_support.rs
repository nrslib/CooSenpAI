...
use crate::tutorial::{SetupAttempt, SetupConnectionMethod};
...
impl DesktopState {
    ...
    pub(crate) async fn test_begin_setup_connection(
        ...
        tutorial.select_setup_provider(provider_name)?;
        tutorial.select_setup_connection_method(SetupConnectionMethod::Login)?;
        let attempt = tutorial.begin_setup_connection(
            provider_name,
    ...
    pub(crate) async fn test_select_setup_language(&self, language: &str) -> anyhow::Result<()> {
        self.tutorial.lock().await.select_setup_language(language)?;
        Ok(())
    }
    ...
    pub(crate) async fn test_commit_stuck_watch_start(
        self: &std::sync::Arc<Self>,
        generation: u64,
        cancellation: &CancellationToken,
    ) -> anyhow::Result<CancellationToken> {
        let (task, worker_cancellation) = crate::watch::WatchTask::stuck_for_test();
        self.commit_watch_start(generation, cancellation, || Ok(task))
            .await?;
        Ok(worker_cancellation)
    }
