...
export function Bubble({ record, revealed, avatarColor, avatarImage, exiting, reading, onHover, controls, onEvent }: Props): ReactElement {
  ...
  >
    ...
    <div className={`speech-bubble${interactive ? " is-interactive" : ""}`} role={controls.bodyButton ? "button" : undefined} tabIndex={controls.bodyButton ? 0 : undefined} onClick={(event) => onEvent({ type: "body", id: record.id, control: isControl(event.target) })}>
      ...
      {record.interaction?.secretInput === undefined ? null : <div className="bubble-secret-input"><label htmlFor={`bubble-secret-input-${record.id}`}>{record.interaction.secretInput.label}</label><input id={`bubble-secret-input-${record.id}`} type={record.interaction.secretInput.action === "utterance-feedback-other" ? "text" : "password"} autoComplete={record.interaction.secretInput.action === "utterance-feedback-other" ? "off" : "new-password"} spellCheck={false} maxLength={record.interaction.secretInput.action === "utterance-feedback-other" ? 500 : undefined} value={secretDraft} placeholder={record.interaction.secretInput.placeholder} disabled={sending} onChange={(event) => setSecretDraft(event.target.value)} /><button type="button" disabled={sending} onClick={() => { onEvent({ type: "secret", id: record.id, value: secretDraft }); }}>{record.interaction.secretInput.submitLabel}</button></div>}
